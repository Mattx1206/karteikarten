// Vercel Serverless Function: liefert Besucherstatistiken + IP-Verlauf fuer das Admin-Dashboard (passwortgeschuetzt)
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const rawLog = r1[4].result || [];
    const ipGesamt = parseInt(r1[5].result || "0", 10);
    const tage = days.map((d, i) => ({ datum: d, besucher: Number(r1[6 + i].result || 0) }));

    const verlauf = rawLog.map(s => { try { return JSON.parse(s); } catch (e) { return null; } }).filter(Boolean);

    let online = [];
    if (onlineSids.length) {
      const r2 = await redis(onlineSids.slice(0, 100).map(s => ["HGETALL", "sess:" + s]));
      online = r2.map((x, i) => {
        const o = parseHash(x.result);
        return {
